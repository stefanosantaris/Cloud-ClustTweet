...
using ClusteringWorkerRole.Similarity;
using MyWebRole;
...
namespace ClusteringWorkerRole
{
    class ClusterHandler
    {
        ...
        public ClusterHandler(string clusterFileName)
        {
            this.clusterFileName = clusterFileName;
            blobConnector = new BlobConnector(Properties.ClusteringContainerName);
            DownloadFile();
            InitializeClusters();
        }
        ...
        {
            if (blobConnector.BlobExist(clusterFileName))
            {
                blobConnector.DownloadFile(clusterFileName);
            }
        }

        private void UploadFile()
        {
            blobConnector.UploadFile(clusterFileName);
        }

        private void InitializeClusters()
        {
            if (!File.Exists(clusterFileName))
            {
                clusterCollection = new ClusterCollection();
            }
            else
            {
                FileStream fs = new FileStream(clusterFileName, FileMode.Open);
                try
                {
        ...
        {
            FileStream fs = new FileStream(clusterFileName, FileMode.Create);
            BinaryFormatter formatter = new BinaryFormatter();
            try
            {
